...
fun LanguageDialog(
    ...
    val selectedLanguageCode = remember { mutableStateOf("en") }
            ...
            TextButton(
                onClick = {
                    coroutineScope.launch {
                        prefsProvider.setLanguage(selectedLanguageCode.value)
    ...
    LaunchedEffect(Unit) {
        val current = LocaleHelper.currentLanguage(context)
        BuildConfig.TRANSLATED_LOCALES.firstOrNull {
            it == current
        }?.let {
            selectedLanguageCode.value = it
        }
    }
}
